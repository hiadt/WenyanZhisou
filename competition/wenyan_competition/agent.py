...
class AcademicSearchAgent:
    ...
    def search(self, query: str, top_k: int = 20, synthesize: bool = True) -> SearchOutput:
        ...
        if self.config.ranking.dense_head_protection:
            before_dense_head = [
            ...
            candidates = merge_baseline_head(
                baseline,
                candidates,
                self.config.ranking.dense_head_size,
            )
            after_dense_head = [
                paper.key() for paper in candidates[: self.config.ranking.dense_head_size]
            ]
            if before_dense_head != after_dense_head:
                self._add_trace(
                    trace,
                    role="Ranker",
                    action="v12 head with dense tail",
                    detail=(
                        "Rank non-dense candidates independently for Top20; "
                        "retain dense-only candidates in Top21-100 for recall."
                    ),
                    candidates_before=len(candidates),
                    candidates_after=len(candidates),
    ...
    def _selector_candidates(self, candidates: List[Paper], limit: int) -> List[Paper]:
        ...
        if self.config.ranking.dense_head_protection:
            baseline = [paper for paper in candidates if not is_dense_only_candidate(paper)]
            dense_only = [paper for paper in candidates if is_dense_only_candidate(paper)]
            candidates = baseline + dense_only
        selected: List[Paper] = []
...
def merge_baseline_head(
    baseline_ranked: List[Paper],
    all_ranked: List[Paper],
    head_size: int,
) -> List[Paper]:
    """Use the v12 candidate stream for the head and dense results for the tail."""

    target = min(max(1, head_size), len(all_ranked))
    head = baseline_ranked[:target]
    selected = {paper.key() for paper in head}
    if len(head) < target:
        for paper in all_ranked:
            ...
            if len(head) >= target:
                break
    return head + [paper for paper in all_ranked if paper.key() not in selected]
